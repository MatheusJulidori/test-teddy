...
import { useSelectedClientList, useClientMutations } from '@/api/modules/clients/hooks';
...
export function SelectedPage() {
  ...
  const { data: clientsResponse, isLoading, error } = useSelectedClientList(page, clientsPerPage);
  const { update, remove, toggleSelect, toggleAllSelected } = useClientMutations();
  ...
  const handleToggleAll = () => {
    ...
    toggleAllSelected.mutate(clientIds);
  };
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-64">
        <div className="text-lg">Loading selected clients...</div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center items-center min-h-64">
        <div className="text-lg text-destructive">
          Error loading selected clients: {error.message}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <h2 className="text-lg">
          <strong>{totalClients}</strong> clientes selecionados:
        </h2>
      ...
      {totalClients === 0 ? (
        <div className="flex justify-center items-center min-h-64">
          <div className="text-lg text-gray-500">
            Nenhum cliente selecionado encontrado.
          </div>
        </div>
      ) : (
        <>
          ...
          <div>
            <Button
              ...
              disabled={toggleAllSelected.isPending}
            >
              {toggleAllSelected.isPending ? 'Removendo seleção...' : 'Remover seleção de todos'}
            </Button>
          </div>
